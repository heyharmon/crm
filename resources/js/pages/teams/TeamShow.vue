        ...
        <div class="container mx-auto py-8 px-4">
            ...
            <div v-else-if="teamStore.currentTeam">
                <div class="flex justify-between items-center mb-8">
                    ...
                    <div class="flex space-x-2">
                        <Button v-if="isOwner || isAdmin" @click="showEditModal = true" class="bg-neutral-200 hover:bg-neutral-300 text-neutral-800">
                            Edit Team
                        </Button>
                        <Button v-if="isOwner || isAdmin" @click="showInviteModal = true" class="bg-blue-600 hover:bg-blue-700 text-white">
                            Invite Member
                        </Button>
                    </div>
                </div>
                ...
                <div v-if="teamStore.pendingMembers.length > 0" class="mt-8 bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
                    ...
                    <div class="divide-y divide-neutral-200">
                        <div v-for="member in teamStore.pendingMembers" :key="member.id" class="px-6 py-4">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    <div class="font-medium">{{ member.name }}</div>
                                    <div class="text-sm text-neutral-500">{{ member.email }}</div>
                                    <div class="text-xs text-neutral-400 mt-1">Invited: {{ formatDisplayDate(member.pivot.invitation_sent_at) }}</div>
                                </div>
                                <div class="flex items-center space-x-4">
                                    <div class="text-sm">
                            ...
                            <div v-if="member.invitation_url" class="mt-3 flex items-center gap-2">
                                <input
                                    :value="member.invitation_url"
                                    readonly
                                    class="flex-1 px-3 py-2 text-sm border border-neutral-300 rounded-md bg-neutral-50 text-neutral-600"
                                />
                                <Button
                                    @click="copyInvitationUrl(member.invitation_url)"
                                    class="bg-neutral-200 hover:bg-neutral-300 text-neutral-800 whitespace-nowrap"
                                >
                                    Copy URL
                                </Button>
                            </div>
            ...
            <div v-if="showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white rounded-lg p-6 w-full max-w-md">
                    ...
                    <div class="flex justify-end space-x-2">
                        <Button @click="showEditModal = false" class="bg-neutral-200 hover:bg-neutral-300 text-neutral-800"> Cancel </Button>
                        <Button @click="updateTeam" :disabled="isSubmitting || !editTeamName" class="bg-blue-600 hover:bg-blue-700 text-white">
            ...
            <div v-if="showInviteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white rounded-lg p-6 w-full max-w-md">
                    ...
                    <div class="flex justify-end space-x-2">
                        <Button @click="showInviteModal = false" class="bg-neutral-200 hover:bg-neutral-300 text-neutral-800"> Cancel </Button>
                        <Button @click="inviteUser" :disabled="isSubmitting || !inviteEmail" class="bg-blue-600 hover:bg-blue-700 text-white">
